<template>
  <div class="dynamic-comment-box van-top-hr">
    <van-row>
      <van-col span="4">
        <van-image :src="iconBgImg" round width="40" height="40"></van-image>
      </van-col>
      <van-col span="18">
      ...
      <van-col span="2">
        <div class="like-box" @click="onLike()">
          <van-icon v-if="likeObj.islike == 1" class="icon-like" name="like" />
...
const emits = defineEmits(['closePopup', 'delItem'])
...
// 是否点赞/点赞数量
const likeObj = ref({} as any)
likeObj.value = {
  islike: props.currItem.islike,
  likes: props.currItem.likes
}

// 格式化点赞数量
const likes = computed(() => {
  const value = likeObj.value.likes
...
const onLike = () => {
  ...
  const commentid = props.currItem.id
}

const deleteItem = (item: any) => {
  const params: any = {
    uid: userinfo.userInfo.id,
    commentid: props.currItem.id,
    commentuid: props.currItem.userinfo.id
  }
  if (props.isVideo) {
    params.service = 'Video.DelComments'
    params.videoid = props.currItem.videoid
  } else {
    params.service = 'Dynamic.DelComments'
    params.dynamicid = props.currItem.dynamicid
  }
}
const onEllipsis = () => {
  if (!userStore.userInfo?.token) {
  ...
  // homeStore.updateReportItem(props.currItem)
  if (router.currentRoute.value.name === 'index') {
    // homeStore.updateIsReport(true)
    return
  }
